...
0. **`Docs/project_timeline.xlsx`** — regenerated by `Scripts/project_timeline_sync.py`, not hand-maintained. Sheet 1 is the full commit history (date/author/message/size); sheets 2–3 are a live snapshot of `requirements.md`'s item statuses and `tools.md`'s "Needed, not yet built" table. Fastest way to answer "what's already been built and when" without re-deriving it from git log or diffing docs by hand — re-run the script after any commit/doc-status change to keep it current.
