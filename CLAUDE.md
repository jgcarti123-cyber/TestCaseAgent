...
See `Docs/requirements.md` items 4–8 (traceability data source, bug history feed, test bench capability list, compliance mapping, golden eval set) and the two pending tasks logged in-session (residual signal discrepancies, `DoorStateDrvr` naming ambiguity) — both need a domain expert, not further agent investigation.
