...
6. **Not a duplicate — exact-field match (current method) — category-scoped as of 2026-08-06.** Flag as duplicate if `parent_id` + `requirement_id` + `test_set_category` + the primary trigger signal (first row of `can_signals_referenced`) all match an existing row in `Existing_TestCases/` or elsewhere in the same generation batch.
   - **This check only blocks release for `Edge Case - *` and `User-Journey` categories.** For `Happy Path` and `Negative Case`, a match is logged for visibility but never blocks — resemblance to existing coverage is expected and correct there, not a defect. Basic/core-requirement test cases exist specifically to be re-verified every build as regression coverage; a "duplicate" happy-path test that fires the same way it did last release is the system working, not accidental waste. Edge cases and user journeys are meant to be novel exploration, so a match there is more likely genuine accidental duplication worth catching. Team decision, 2026-08-06 — see the worked resolution below.
   ...
   - **Scope extended 2026-08-06**: this same semantic-similarity check, once built, also needs to run against `Traceability/requirement_traceability.json`'s `features.*.linked_test_issues` summaries (real Jira Test issue titles), not just `Existing_TestCases/` — and the same category scoping applies: only block on Edge Case/User-Journey matches, log-only for Happy Path/Negative Case. Jira's traceability tops out at feature-level, not per-requirement-line, so scenario-level similarity against real Jira test summaries is the chosen way to catch likely duplicates there — see `traceability.md` → Granularity gap decision. A naive keyword-overlap check was tried and rejected: it produced a false 75% "match" between two unrelated test cases purely from shared boilerplate words, confirming this needs real embeddings, not a bag-of-words heuristic.
   - **Resolved by this rule**: the `NIV-1595`/`Test_90` and `NIV-1586`/`Test_91` overlaps noted when the Jira data first came in are exactly Happy Path and Negative Case — under this rule they're expected regression-style overlap, not a concern requiring human confirmation. No action needed on those two.
