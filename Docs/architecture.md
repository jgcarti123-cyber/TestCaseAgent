...
6. **Coverage & Dedupe Checker** — three checks, not one, since neither local nor Jira data alone is enough:
   ...
   **All three are category-scoped, not blanket (2026-08-06):** a match only blocks release for `Edge Case - *` and `User-Journey` test cases. For `Happy Path` and `Negative Case`, a match is expected — these are core-requirement regression tests meant to recur every build, not accidental waste — so the checker logs it for visibility and lets it through rather than rejecting. The Test Case Generator (stage 5) should keep producing Happy Path/Negative Case cases even when the feature already has dense existing coverage; regression coverage isn't optional just because it isn't novel.

   Enforced by tools, never by asking the LLM "did you check?"
